...
\begin{itemize}
  ...
  \item \textbf{Neyman allocation:} choose \(\pi_{\ast}\) to minimize asymptotic
    ...
    \end{equation*}
    Intuition: assign higher proportion to arm that is more difficult to learn
    about.
    \vfill
...
\begin{itemize}
  ...
  \item If \(\tilde{\sigma}_{m}^{2} (a) \pto \sigma^{2} (a)\) as \(m \to
    ...
    \end{equation*}
    \vfill
  \item If pilot is large, \(\hat{\theta}\) with FNA has efficiency close to
    infeasible NA.
\end{itemize}
\end{frame}
...
\textbf{Note:} \(B_{m} \geq 1\) can be shown by AM-GM inequality ---
...
\begin{equation*}
  \mathcal{L}^{d} (F) =
  \E \left[ \frac{\sigma^{2} (1)}{\tilde{\pi}} + \frac{\sigma^{2} (0)}{1 -
  \tilde{\pi}} \right] - 2 (\sigma^{2} (1) + \sigma^{2} (0)), \qquad
  \mathcal{L}^{r} (F) = \frac{\E \left[ \frac{\sigma^{2} (1)}{\tilde{\pi}} +
  \frac{\sigma^{2} (0)}{1 - \tilde{\pi}} \right]}{2(\sigma^2(1)+\sigma^2(0))}.
\end{equation*}
\end{defn*}
\vfill
\begin{prop*}
Under fixed-\(m\) asymptotics, with
\(\mathcal{F}(K) = \{F:\, 0 < \sigma^2(0),\sigma^2(1) \le K\}\):
\begin{align*}
  \sup_{F \in \mathcal{F}(K)}\!\mathcal{L}^d(F) &= +\infty,
   & \inf_{F \in \mathcal{F}(K)}\!\mathcal{L}^d(F) &= -K, \\[2pt]
  \sup_{F \in \mathcal{F}(\infty)}\!\mathcal{L}^r(F) &= +\infty,
   & \inf_{F \in \mathcal{F}(\infty)}\!\mathcal{L}^r(F) &= \tfrac{1}{2}.
\end{align*}
\end{prop*}
\vfill
In terms of maximum regret, BA dominates FNA.
\end{frame}
...
\(\tilde{\pi}^{-1} = 1 + (x \tilde{Z}_{m})^{-1}\) and \((1 -
\tilde{\pi})^{-1} = 1 + x \tilde{Z}_{m}\).
